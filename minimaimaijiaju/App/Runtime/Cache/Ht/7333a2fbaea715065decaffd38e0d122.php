      ...
      <table class="pro_3">
         ...
         <!-- 遍历 -->
          <?php if(is_array($shangchang)): $i = 0; $__LIST__ = $shangchang;if( count($__LIST__)==0 ) : echo "暂时没有店铺，赶紧进行招商吧" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr data-id="<?php echo ($v["id"]); ?>" data-name="<?php echo ($v["name"]); ?>">
             <td><?php echo ($v["id"]); ?></td>
             <td><img src="/minibuysalejiaju/Data/<?php echo ($v["logo"]); ?>" class="img_1"/></td>
             ...
             <td class="obj_1">
              <a href="<?php echo U('set_user');?>?id=<?php echo ($v["id"]); ?>">账号设置</a> | 
              <a href="<?php echo U('set_tj');?>?id=<?php echo ($v["id"]); ?>">推荐</a> |
