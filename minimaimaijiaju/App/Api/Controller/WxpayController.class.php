...
class WxpayController extends Controller{
	...
	public function wxpay(){
		...
		$notify_url=C("weixin.notify_url");
		//②、统一下单
		$input = new \WxPayUnifiedOrder();
		$input->SetBody("商品_".trim($order_info['order_sn']));
		$input->SetAttach("商品_".trim($order_info['order_sn']));
		$input->SetOut_trade_no($pay_sn);
		$input->SetTotal_fee(floatval($order_info['amount'])*100);
		$input->SetTime_start(date("YmdHis"));
		$input->SetTime_expire(date("YmdHis", time() + 3600));
		$input->SetGoods_tag("商品_".trim($order_info['order_sn']));
	...
	public function orderhandle($data){
		...
		if ($res) {
			...
			//赠送 电子体验券
			$resup = $this->getcoupons($check_info);
			if (is_array($resup)) {
				return array('status'=>1,'data'=>$data);
			}else{
				return $resup;
