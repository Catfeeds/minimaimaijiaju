...
class ProductController extends PublicController {
    ...
    public function index(){
		...
		$sheng = M('shangchang')->where('id='.intval($pro['shop_id']))->getField('sheng');
		$city = M('shangchang')->where('id='.intval($pro['shop_id']))->getField('city');
		$quyu = M('shangchang')->where('id='.intval($pro['shop_id']))->getField('quyu');
		$pro['address'] = M('china_city')->where('id='.intval($sheng))->getField('name').' ';
		$pro['address'] .= M('china_city')->where('id='.intval($city))->getField('name').' ';
		$pro['address'] .= M('china_city')->where('id='.intval($quyu))->getField('name');
		if(intval($pro['protype']) == 2){
			$pro['protype'] = '定制';
		}else{
			$pro['protype'] = '现货';
		}
		...
		//获取产品优惠券
		$temp_vou = M('voucher')->where('start_time<'.time().' AND end_time>'.time())->select();
		$vou = array();
		if($temp_vou){
			foreach($temp_vou as $k => $v){
				if($v['proid'] == 'all'){
					array_push($vou,$v);
				}else{
					$proid = explode(',',$v['proid']);
					if(in_array(intval($pro_id),$proid)){
						array_push($vou,$v);
					}
				}
		...
		echo json_encode(array('status'=>1,'pro'=>$pro,'buff'=>$buff,'vou'=>$vou,'server'=>$server));
		exit();
