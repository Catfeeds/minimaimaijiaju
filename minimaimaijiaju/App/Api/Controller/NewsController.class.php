...
class NewsController extends PublicController {
    ...
    public function index(){
        ...
        $list = M('news')->where($where)->field('id,digest,name,photo,addtime,source,click')->order('sort desc,addtime desc')->limit(8)->select();
        foreach ($list as $k => $v) {
    ...
    public function getlist(){
        ...
        $list = M('news')->where($where)->field('id,digest,name,photo,addtime,source,click')->order('sort desc,addtime desc')->limit($limit.',8')->select();
        foreach ($list as $k => $v) {
            $list[$k]['photo']=__DATAURL__.$v['photo'];
            $list[$k]['addtime']=date('Y-m-d',$v['addtime']);
        }
